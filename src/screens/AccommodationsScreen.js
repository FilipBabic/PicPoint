...
const AccommodationScreen = ({ route, navigation }) => {
    ...
    const [accommodationImages, setAccommodationsImages] = useState({});
    ...
    const getNearByPlaces = async (latitude, longitude) => {
        fetch('https://maps.googleapis.com/maps/api/place/nearbysearch/json?location=' + latitude + ',' + longitude + '&keyword=lodging&rankby=distance&key=' + GoogleMapsAPIKey)
    ...
    useEffect(() => {
        ...
        accommodationPlaces.forEach((item) => {
            if (item.photos == null) {
                setAccommodationsImages((oldState) => ({
                    ...oldState,
                    [item.place_id]: null
                }));
                return;
    ...
    const renderImage = (img) => {
        if (img === null) {
            return <Image source={AccommodationBackground} style={{ height: 120, width: 120 }} />;
        }
    ...
    return isLoading === false ? (
        <ScrollView>
            <View style={{ backgroundColor: '#f2fbfc' }}>
                <Text style={{ fontFamily: 'Poppins-Bold', fontSize: 16, paddingTop: 10, textAlign: 'center', fontWeight: '800' }}>
                    Find Lodging nearby
                </Text>
                            ...
                            <View key={index} style={{
                                ...
                                <TouchableOpacity onPress={() => {
                                    ...
                                    <View style={{ borderWidth: 1 }}>
                                        {renderImage(accommodationImages[element.place_id])}
                                        <View style={{ position: 'absolute', paddingTop: 5, paddingBottom: 5, paddingLeft: 10, paddingRight: 10, top: 0, left: isOpen ? '35%' : '30%', backgroundColor: isOpen ? 'green' : 'red', borderBottomLeftRadius: 10, borderBottomRightRadius: 10 }}>
                                            <Text style={{ fontFamily: 'Poppins-Bold', color: 'white', fontSize: 15 }}>
